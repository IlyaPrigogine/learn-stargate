...
contract StargateFeeLibraryV02 is IStargateFeeLibrary, Ownable, ReentrancyGuard {
    ...
    function getFees(uint256 _srcPoolId, uint256 _dstPoolId, uint16 _dstChainId, address, uint256 _amountSD) external view override returns (Pool.SwapObj memory s) {
    ...
    function _getEquilibriumFee(uint256 idealBalance, uint256 beforeBalance, uint256 amountSD) internal pure returns (uint256, uint256) {
